...
export default function FineiasPage() {
  ...
  return (
    <div className="flex flex-col bg-cedro-white text-cedro-ink">
      ...
      {/* O CICLO */}
      <section className="py-24 bg-[#f6f2ea] text-cedro-ink relative overflow-hidden" style={textureStyle}>
        <div className="max-w-[1200px] mx-auto px-5 md:px-8 relative z-10">
          <h2 className="mb-12 text-cedro-navy">O ciclo que você conhece bem demais.</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 my-8">
            {[
              ...
              <div key={i} className="p-8 bg-white border-l-4 border-cedro-red rounded-r-lg shadow-sm">
                <h4 className="font-sans text-base font-bold text-cedro-navy mb-2">{symptom.title}</h4>
                <p className="text-sm text-cedro-ink/70">{symptom.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1200px] mx-auto px-5 md:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h2 className="mb-4 text-cedro-navy">
                A Sociedade Finéias não é um grupo de apoio onde você fica repetindo que é viciado.
              </h2>
              <Divider />
              <p className="mb-4 text-cedro-ink/72">É uma mentoria de transformação onde você muda o eixo narrativo: da culpa para o chamado, do vício para a filiação.</p>
              <p className="text-cedro-ink/72"><strong className="text-cedro-navy font-bold">Mentalidade Finéias:</strong> não é só resistir ao pecado. É odiar o que destrói a aliança e agir com zelo.</p>
            </div>
            <div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 my-12">
                <div className="text-center p-8 border border-cedro-clay/12 bg-[#f8f4ec] rounded-lg">
                  <div className="font-serif text-5xl font-bold text-cedro-red">10</div>
                  <p className="text-sm text-cedro-ink/65 mt-2">Encontros online</p>
                </div>
                <div className="text-center p-8 border border-cedro-clay/12 bg-[#f8f4ec] rounded-lg">
                  <div className="font-serif text-5xl font-bold text-cedro-red">12</div>
                  <p className="text-sm text-cedro-ink/65 mt-2">Homens por turma</p>
                </div>
                <div className="text-center p-8 border border-cedro-clay/12 bg-[#f8f4ec] rounded-lg">
                  <div className="font-serif text-5xl font-bold text-cedro-red">0</div>
                  <p className="text-sm text-cedro-ink/65 mt-2">Gravações</p>
                </div>
              </div>
              <p className="text-sm text-cedro-ink/65 text-center">Tudo é ao vivo, em tempo real, com aliança e confidencialidade.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* CTA MEIO */}
      <section className="py-12 bg-cedro-navy border-y border-cedro-red/30 text-center">
        <div className="max-w-[1200px] mx-auto px-5 md:px-8">
      ...
      {/* 10 ENCONTROS */}
      <section className="py-24 bg-cedro-white text-cedro-ink relative overflow-hidden" style={textureStyle}>
        <div className="max-w-[1200px] mx-auto px-5 md:px-8 relative z-10">
          <SectionLabel>Os 10 encontros</SectionLabel>
          <h2 className="mb-8 text-cedro-navy">A jornada completa.</h2>
          <div className="my-12">
            {encounters.map((encounter, i) => (
              <div key={i} className="grid grid-cols-[80px_1fr] gap-6 py-6 border-b border-cedro-clay/15">
                <div className="font-serif text-4xl text-cedro-red/60">{encounter.num}</div>
                <div>
                  <h4 className="font-sans text-base font-bold text-cedro-navy mb-1">{encounter.title}</h4>
                  <p className="text-sm text-cedro-ink/70">{encounter.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 bg-[#f6f2ea] text-cedro-ink relative overflow-hidden" style={textureStyle}>
        <div className="max-w-[1200px] mx-auto px-5 md:px-8 relative z-10">
          ...
          <h2 className="mb-12 text-cedro-navy">Homens que pararam de se esconder.</h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Elias */}
            <div className="p-8 bg-white border border-cedro-clay/12 border-l-[3px] border-l-cedro-red rounded-r-lg flex flex-col shadow-sm">
              <p className="text-cedro-ink/70 text-[0.95rem] leading-relaxed mb-6 flex-1">
              ...
              <div className="pt-4 border-t border-cedro-clay/15">
                <p className="text-cedro-navy font-bold text-sm">Elias</p>
                <p className="text-cedro-ink/60 text-xs">36 anos</p>
              </div>
            </div>

            {/* Denis */}
            <div className="p-8 bg-white border border-cedro-clay/12 border-l-[3px] border-l-cedro-red rounded-r-lg flex flex-col shadow-sm">
              <p className="text-cedro-ink/70 text-[0.95rem] leading-relaxed mb-4 flex-1">
              ...
              <div className="pt-4 border-t border-cedro-clay/15">
                <p className="text-cedro-navy font-bold text-sm">Denis</p>
                <p className="text-cedro-ink/60 text-xs">45 anos, engenheiro mecânico</p>
              </div>
            </div>
            ...
            <div className="p-8 bg-white border border-cedro-clay/12 border-l-[3px] border-l-cedro-red rounded-r-lg flex flex-col shadow-sm">
              ...
              <div className="pt-4 border-t border-cedro-clay/15">
                <p className="text-cedro-navy font-bold text-sm">Lucas</p>
                <p className="text-cedro-ink/60 text-xs">23 anos</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-cedro-white text-cedro-ink relative overflow-hidden">
        <div className="max-w-[1100px] mx-auto px-5 md:px-8 relative z-10">
          <SectionLabel>Investimento e turma</SectionLabel>
          <h2 className="mb-12 text-cedro-navy">O que está em jogo.</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
            {/* Card preço */}
            <div className="p-10 bg-[#f8f4ec] border border-cedro-red/25 rounded-lg relative overflow-hidden">
              ...
              <p className="text-xs font-bold tracking-[0.3em] uppercase text-cedro-ink/55 mb-4">Investimento</p>

              <p className="text-cedro-ink/60 text-base mb-2">
                <span className="line-through opacity-70">De R$ 1.497</span>
              </p>
              <div className="flex items-baseline gap-2 mb-6">
                <span className="text-cedro-ink/60 text-lg">por</span>
                <span className="font-serif font-bold text-cedro-red leading-none" style={{ fontSize: 'clamp(3rem, 6vw, 4.5rem)' }}>
                  R$ 1.197
                </span>
              </div>
              <p className="text-cedro-ink/65 text-sm leading-relaxed mb-2">à vista</p>
              <div className="w-12 h-[2px] bg-cedro-red my-6" />
              <p className="text-cedro-ink/65 text-sm leading-relaxed">
                Acesso completo aos 10 encontros ao vivo. Sem gravação, sem upsell, sem mensalidade.
              </p>
            </div>

            {/* Card turma */}
            <div className="p-10 bg-[#f8f4ec] border border-cedro-clay/15 rounded-lg flex flex-col">
              <p className="text-xs font-bold tracking-[0.3em] uppercase text-cedro-ink/55 mb-4">Próxima turma</p>
              <h3 className="text-cedro-navy text-2xl font-bold mb-6 leading-tight">
                Começa quando fechar 12 homens.
              </h3>
              <p className="text-cedro-ink/70 text-[0.95rem] leading-relaxed mb-4">
              ...
              <p className="text-cedro-ink/70 text-[0.95rem] leading-relaxed mb-8">
                Você entra na lista, recebe o aviso quando fechar, e decide se entra ou não.
              </p>
              <div className="mt-auto pt-6 border-t border-cedro-clay/15 grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="font-serif text-3xl font-bold text-cedro-red leading-none">12</div>
                  <p className="text-xs text-cedro-ink/60 mt-2">Homens</p>
                </div>
                <div>
                  <div className="font-serif text-3xl font-bold text-cedro-red leading-none">10</div>
                  <p className="text-xs text-cedro-ink/60 mt-2">Encontros</p>
                </div>
                <div>
                  <div className="font-serif text-3xl font-bold text-cedro-red leading-none">0</div>
                  <p className="text-xs text-cedro-ink/60 mt-2">Gravações</p>
                </div>
              </div>
            </div>
          </div>
